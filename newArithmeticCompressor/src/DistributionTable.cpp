...
freqDT DistributionTable::getCummulativePreviousCharacterFrequency(charDT currentC) {
...
void DistributionTable::addCharacter(charDT c, freqDT frequency) {
	int sizedt=distributionTable.size();
...
void DistributionTable::removeCharacters(std::vector<unsigned char> v) {
...
std::string DistributionTable::toHuman()
{
	...
	for (it = distributionTable.begin(); it != distributionTable.end(); it++) {
		...
		if(c == C_EOF)
			...
			humanTable.append(1,(unsigned char)c);
