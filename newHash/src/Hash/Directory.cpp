 ...
 */
bool Directory::insert (Key key, string value){
...
void Directory::duplicateTable(unsigned int pos,Offset offset){
	...
	unsigned int changePos = pos + (LIMIT_TABLE* table->getOffset());
    ...
    while(!Cicle){
        ...
        for(i = 0;i < inter;i++){
        	if (++actualPos >= table->getSize() ){
        		actualPos = 0;
        		if (++offTable >= dirFile->blocks()) offTable = 0;
        	}
        }
 ...
 */
unsigned int Directory::findPosofTable (unsigned int key){

	Offset offset= 0;
	unsigned int acount= 0;
	unsigned int pos = HashFunction::MakeAddress(key,this->depth);
...
void Directory::inform (){
	...
	for (Offset block= 0 ; block < this->dirFile->blocks() ; block++){
	...
	for (std::set<Offset>::iterator it = offsets.begin(); it != offsets.end(); it++){
		this->bktFile->load(*it,this->bucketActual);
		std::cout << "	Bucket "<< *it << ": (" << this->bucketActual->getDepth() << "," << this->bucketActual->countElements() << ")  =>";
