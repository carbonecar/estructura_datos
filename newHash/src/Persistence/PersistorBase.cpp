...
void PersistorBase::add(Block* block) {
	Offset offset = freeSpaceManager->pop();
...
std::string PersistorBase::load(Offset offset){
    /**
     * verifica que el archivo esta abierto y
     * arroja una excepcion si no lo esta.
     * */
	...
	/* Se Posiciona en en inicio del Block */
	if (!freeSpaceManager->isBlockUsed(offset)) throw new PersistExceptions::ElementNotFoundException();

	archivo.seekg((offset * blockSize) + HEADER_SIZE);
	...
	std::string buffer="";
	buffer.append(charBuffer,blockSize);
...
void PersistorBase::empty(){
	...
	/* Se elimina el archivo actual, y se crea de nuevo. */
	remove(this->fileName.c_str());
	this->newFile(this->fileName);
	remove(this->fileName.append(".fs").c_str());
}
