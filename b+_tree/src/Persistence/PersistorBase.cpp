...
PersistorBase::PersistorBase(std::string fileName, BlockSize size) throw() {
...
void PersistorBase::newFile(std::string fileName) throw(){
	/* crea el archivo */
...
void PersistorBase::modify(Block* block) throw(){
    /**
	...
	if (this->archivo.fail())
		throw new FailFileWriteException();
}

void PersistorBase::load(Offset key, Block* block) throw(){
	std::string temp = this->load(key);
	block->unserialize(temp);
	block->setOffset(key);
}

std::string PersistorBase::load(Offset key) throw(){
    /**
     * verifica que el archivo esta abierto y
     * arroja una excepcion si no lo esta.
     * */
	...
	/* Se Posiciona en en inicio del Block */
	if (!freeSpaceManager->isBlockUsed(key)) throw new ElementNotFoundException();
...
void PersistorBase::empty() throw(){
	this->deleteFile();
